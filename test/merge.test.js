...
describe("merge", function () {
  ...
  it("should not mutate the source when the object has a class", function () {
    class Foo {
      b = 1;
    }

    const source1 = { a: new Foo() };
    const source2 = { a: { b: 2 } };

    assert.equal(JSON.stringify(source1), JSON.stringify({ a: { b: 1 } }));

    const dest = merge({}, source1, source2);

    assert.equal(JSON.stringify(dest), JSON.stringify({ a: { b: 2 } }));
    assert.equal(JSON.stringify(source1), JSON.stringify({ a: { b: 1 } }));
  });
});
